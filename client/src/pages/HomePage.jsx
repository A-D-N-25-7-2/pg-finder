...
const HomePage = () => {
  ...
  return (
    <div className="min-h-screen bg-dark-base">
          ...
          <div className="flex gap-2 justify-center flex-wrap mt-8 animate-fade-in" style={{ animationDelay: "0.3s" }}>
            {["Bangalore", "Mumbai", "Pune", "Delhi", "Hyderabad"].map((c) => (
              <button
                ...
                className="px-4 py-1.5 rounded-full text-sm font-medium text-gray-400 border border-dark-border hover:border-blue-500/40 hover:text-blue-400 hover:bg-blue-500/10 transition-all"
              >
                {c}
              </button>
            ))}
      ...
      {/* ── Stats ────────────────────────────────────────── */}
      <div className="border-y border-dark-border bg-dark-card/50">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {[
            ...
            >
              ...
              <p className="mt-4 font-semibold text-blue-400 group-hover:text-blue-300 transition-colors">
                Browse {item.type}s →
              </p>
            </button>
          ))}
      ...
      {/* ── Browse by Gender ─────────────────────────────── */}
      <div className="bg-dark-card/50 border-y border-dark-border py-16 px-4">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex gap-4 justify-center flex-wrap">
            {[
              { label: "Male PGs", gender: "Male", icon: "👨", color: "border-blue-500/30 text-blue-400 hover:bg-blue-500/10" },
              { label: "Female PGs", gender: "Female", icon: "👩", color: "border-pink-500/30 text-pink-400 hover:bg-pink-500/10" },
              { label: "Any Gender", gender: "Any", icon: "🤝", color: "border-emerald-500/30 text-emerald-400 hover:bg-emerald-500/10" },
            ].map((item) => (
              <button
                key={item.gender}
      ...
      <div className="max-w-5xl mx-auto px-4 py-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {features.map((f, i) => (
            <div
              key={f.title}
              className={`bg-dark-card border border-dark-border rounded-2xl p-6 text-center card-glow animate-fade-in stagger-${i + 1}`}
              style={{ opacity: 0 }}
            >
